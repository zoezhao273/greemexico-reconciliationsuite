...
- Aggregates `IVA (NCC)` (`VATAmt(FC)`) and collects `IVA Rate (NCC)` (`* VATCode`) per row.
- **Enrichment:** when the UUID is a well-formed folio **and** found in the CFDI file, every other field is taken from the CFDI: `Emisor RFC`, `Estatus`, `Base IVA 16`, `Base IVA 8`, `Base IVA 0`, `Base IVA Exento`, `Descuento`, `IVA`, `IVA Retenido`, `ISR Retenido`. Otherwise (non-standard UUID, or folio not in the file) the `UUID` column shows the NCC value **together with its DocNo. and Material** (e.g. `26 16 3167 6015444 | PY-MX-2605-00183 | Custom Clearance - Service`) so the line stays traceable, and every CFDI-sourced field is left blank.
...
- Outputs the report columns in sequence — **IVA (NCC) · IVA Rate (NCC) · Emisor RFC · UUID · Estatus · Base IVA 16 · Base IVA 8 · Base IVA 0 · Base IVA Exento · Descuento · IVA · IVA Retenido · ISR Retenido · Note** — with a flag panel for matched-but-not-`Vigente` (e.g. Cancelado) invoices, exportable to Excel.
